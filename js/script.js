...
function cetakDokumen() {
  ...
  const opsi = {
    ...
    // TAMBAHKAN PENGATURAN DI BAWAH INI UNTUK MEMATIKAN CACHE & DUPLIKASI HTML
    html2canvas:  { 
      scale: 2, 
      useCORS: true, 
      logging: false,
      removeContainer: true // Otomatis menghapus container sampah setelah cetak selesai
    },
  ...
  html2pdf().set(opsi).from(elemen).save().then(() => {
    // BERSIHKAN ELEMENT DOKUMEN CETAK AGAR KEMBALI SEPERTI SEMULA
    // Ini jurus ampuh agar cetakan kedua dan seterusnya tidak membawa sisa spasi cetakan pertama
    html2pdf().set(opsi).from(elemen).toContainer().clean(); 
